# 第 15 关：ESP32 挂载 MicroSD/TF 卡 (4-bit SDIO) 与 FATFS 文件系统
...
## 15.1 为什么板载 Flash 不够用？TF 卡是单片机的“移动硬盘”
...
## 15.2 什么是 4-bit SDIO / SDMMC？为什么比 SPI 读卡快 4 倍？
...
## 15.4 实战第 1 步：TF 卡 4-bit SDMMC 高速挂载与读写 (Mount SD Card)

> 📁 **配套源码文件**：[`code/15_sdcard_fatfs/01_sdcard_mount.c`](../code/15_sdcard_fatfs/01_sdcard_mount.c)  
...
## 15.5 实战第 2 步：传感器黑匣子 CSV 数据记录仪 (Data Logger)

模拟工业级传感器黑匣子，周期性将环境气温与内存状态按标准的逗号分隔 CSV 格式追加写入文件：

> 📁 **配套源码文件**：[`code/15_sdcard_fatfs/02_sensor_datalogger.c`](../code/15_sdcard_fatfs/02_sensor_datalogger.c)  
...
## 15.6 实战第 3 步：综合大工程 —— TF 卡文件资源浏览器与目录树遍历

使用 POSIX 标准的 `opendir()` 和 `readdir()` 递归扫描并打印出整张 TF 卡的文件目录树：

> 📁 **配套源码文件**：[`code/15_sdcard_fatfs/03_sd_file_explorer.c`](../code/15_sdcard_fatfs/03_sd_file_explorer.c)  
...
请翻开 [**第 16 章：ESP32 低功耗电源管理与 Deep-sleep 深度睡眠唤醒**](./16_低功耗电源管理与DeepSleep.md)！
